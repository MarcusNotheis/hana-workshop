...
module.exports = function() {
	...
	app.get("/json", (req, res) => {
		...
		const stmt = new Promise((resolve, reject) => {
			client.prepare("SELECT * FROM \"PO.Header\" LIMIT 10", (err, statement) => {
				if (err) {
		...
		stmt.then(statement => exec(statement))
			.then(response => res.type("application/json").status(200).send(JSON.stringify(response)))
		    .catch(error => res.type("text/plain").status(500).send(error.message));
